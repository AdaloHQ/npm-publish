...
var require_cjs = __commonJS({
  "node_modules/simple-git/dist/cjs/index.js"(exports, module2) {
    ...
    function completionDetectionPlugin({
      ...
      function configureTimeout(flag, event2, timeout) {
        if (flag === false) {
          return;
        }
        (flag === true ? event2.promise : event2.promise.then(() => delay(flag))).then(timeout.done);
      }
      return {
        type: "spawn.after",
        action(_0, _1) {
    ...
    function gitP(...args) {
      ...
      return promiseApi;
      function asyncWrapper(fn, git22) {
        return function(...args2) {
          ...
          return chain.then(function() {
            return new Promise(function(resolve, reject) {
              ...
              args2.push(callback);
              git22[fn].apply(git22, args2);
            });
          });
        };
      }
      function syncWrapper(fn, git22, api) {
        return (...args2) => {
          git22[fn](...args2);
          return api;
        };
...
var getlog = promisify2(git.log.bind(git));
var event = github.context.payload;
var deployDir = path.join(process.cwd(), process.env.DEPLOY_DIR || "./");
var srcPackageDir = path.join(process.cwd(), process.env.SRC_PACKAGE_DIR || "./");
console.log("            using deploy directory : " + deployDir);
console.log("using src directory (package.json) : " + srcPackageDir);
var pkg = require(path.join(deployDir, "package.json"));
var run = async () => {
  if (!process.env.NODE_AUTH_TOKEN)
    throw new Error("Merge-release requires NODE_AUTH_TOKEN");
  let latest;
  try {
  ...
  if (latest) {
    ...
    if (latest.gitHead) {
      try {
        ...
        messages = logs.all.map((r2) => r2.message + "\n" + r2.body);
      } catch (e2) {
        latest = null;
      }
    } else {
  ...
  const setVersion = (version2) => {
    const json = execSync(`jq '.version="${version2}"' package.json`, { cwd: srcPackageDir });
    fs2.writeFileSync(path.join(srcPackageDir, "package.json"), json);
    if (deployDir !== "./") {
      const deployJson = execSync(`jq '.version="${version2}"' package.json`, { cwd: deployDir });
      fs2.writeFileSync(path.join(deployDir, "package.json"), deployJson);
    }
  };
  let currentVersion;
  try {
